...
在第二章中我们通过假设各组样本点的分布情况，推出了似乎十分厉害的逻辑回归，但是我们也提到了，线性分类之所以叫线性分类，就是因为他没有办法分类非线性可分的样本点，为了能够区分此类样本，深度学习提供了一种神经网络的结构，通过叠加多层感知机达到非线性划分的目的，但是在神经网络还没广泛流行起来的时期，说起非线性分类问题，就不得不提到在当时大名鼎鼎的SVM算法了
...
$$
其中的$\alpha$在这种正向推导的过程中其实我们完全可以像是在学习指数族分布的$A(\eta)$一样将它理解成一个配分函数，在这里只是为了满足这个模型的概率密度函数在定义域积分为$1$而已，与此同时他还有控制每个模型的权重的作用（比如男人比女人会多一些，那么给男人的高斯分布的权重就可以大一些）

> 你可能注意到了，将其理解成一个配分函数还是比较牵强的，毕竟如果真的要配分你完全可以给每一项乘一个$\frac{1}{N}$，在后面的隐变量问题中，我们会从另一种角度理解这里的$\alpha$
...
我们通过高斯混合模型的求解过程引导出了我们在求解方法上遇到的问题，这种模型也确定了，评价函数也没什么问题，唯独在最后的求解方法上卡住的感觉一定不好受，为了解决这个问题，我们不妨先忘掉刚刚的高斯混合模型，我们从拆解问题本身来重新认识一遍这个问题

我们发现，在这个问题中，男生和女生两个类别是样本没有提供给我们的，或者换种说法，男生和女生只是我们在知道了样本是什么东西之后根据经验得出来里面可能有两个不同的高斯分布，并且假设决定这两个高斯分布的因素是性别而已，我们不妨把我们假设出来的这两个类别叫做隐变量，而我们在指定模型的前提下对这两个隐变量进行估计模型参数的过程有另一个特别的名字，也就是隐变量估计问题

同时需要注意，之所以叫隐变量估计问题，原因就是我们存在两组变量，第一个当然是抽样出这些样本的每个模型参数，而第二个就是我们每一个样本属于的类别隐变量

细心的小伙伴可能发现了，没有标签，根据先验信息进行的模型参数预测问题，这怎么和无监督学习那么像，没错，隐变量估计问题下的每一种解决方法，我们都可以称之为广义上的无监督学习

如果说在高斯混合模型中我们是通过模型去构造极大似然估计，那么在这一节，我们则是要直接从极大似然估计入手，通过纯概率变换的方法导出隐变量估计的评价函数，其实最后你会发现无论从哪个角度理解，最后的公式其实都是差不多的，**混合高斯模型的似然问题只是隐变量估计问题中的一个特例而已**
...
我们确实导出了隐变量估计的式子，但是实际上你会发现，我们通过反推的方法也就是纯数学推导出的式子似乎和我们之前从实际角度推导出的高斯混合模型的式子还是挺不一样的，因此此处我们还需要花一些时间从GMM的公式统一到隐变量估计，首先我们重新写一遍我们把高斯混合模型的模型表达式
$$
p(x)=\sum_{i=1}^N\alpha_i\mathcal{N}(\mu_i,\sigma_i),\sum_{i=1}^N\alpha_i=1
$$
其实在刚推导出这个公式的时候我就提到了，这里的$\alpha$的含义似乎不只是配方那么简单，实际上你发现这东西的累和是$1$，那为什么我不把它看作是一个离散的概率模型呢，你可以假设有一个可以取到$N$个值的骰子，每一个$a_i$就是这个骰子骰到$i$时的概率值，显然的，骰出所有数的概率相加正是为$1$

不过问题就来了，这个概率模型针对的是什么事件呢？我们发现，随着每一个$\alpha$取值的不同，似乎后面的高斯模型的参数也就不一样，那可不可以说，这个$\alpha$控制的正是每一个高斯模型被选取的概率呢，我们不妨就用$z$表示这个事件，于是公式可以改写成
$$
\begin{align*}
p(x)&=\sum_{i=1}^N\alpha_i\mathcal{N}(\mu_i,\sigma_i)\\
&=\sum_{i=1}^Np(z_i)q(x\mid z_i)\\
\end{align*}
$$
由于现在我们有了控制高斯分布内容的$z$，因此我们可以抛去所有的参数$\mu,\sigma$，把所有的高斯模型合在一起想象成一个概率密度函数$q$（尽管我们可能写不出来这个密度函数的表达式），但是又由于我们知道了是由$z_i$控制的高斯模型，意味着只需要一个条件概率，我们就能从一个合在一起的高斯模型密度函数$q$中抽取出我们需要的那个高斯模型，接着输入样本$x$就如同输入进了一个正常的高斯模型中，也就是原表达式中的$\mathcal{N}(\mu_i,\sigma_i)$

但是到目前为止，我们还没有规定$p$这个模型长什么样，现在，我们为什么不妨假设$p(z)=q(z)$，这时候，我们发现和我们推导出的隐变量估计的模型就已经基本上一样了，即
$$
\sum_{i=1}^Np(z_i)q(x\mid z_i)=\sum_zq(x,z)
$$
由于我们可以说我们不知道$q$的函数长什么样，但是显然里面是有参数的，因此自然的，我们可以把其中的参数设为$\theta$，至此，我们完成了最终的概率密度函数，再接上极大似然估计，就变成了我们上面推导过的隐变量估计式子
$$
\hat\theta = \arg\max_\theta\sum^n_{i=1}\log\sum_z p(x_i,z\mid\theta)
$$
...
在正式推导em算法的正确性之前，还需要插播一些数学知识，如果你还记得，在引入KKT条件的时候，我们曾经引入过凸函数的定义，回顾一下，我们提到当某一个函数上的任意两点之间的连线值始终大于这两点之间的函数值，则我们就说这个函数是一个凸函数，或者更通俗的理解，函数图像画出来始终是呈现一个碗一样的形状的函数就是凸函数（下凸函数），他的代数定义如下
$$
...
于是右边的这个式子就叫做左边式子的ELBO，全称是Evidence Lower BOund，也叫证据下界，举个通俗但不那么恰当的例子，我们都知道$x^2+y^2\ge2xy$，此时我们就可以把$2xy$看作是$x^2+y^2$的一个下界（ELBO也就可以看作是专指概率密度情况下的下界），而一旦存在某一个约束使$x=y$，可以发现，这个约束将$2xy$这个下界抬升到了它能够达到的最大的情况，也就是不等式的左右恒等的情况，这也是我们将要做的
...
围绕着求解似然问题的三个步骤，我们在上文中花了大量的篇幅介绍了在不同的场景下每个步骤中非常典型解决方案，每一种方案的组合都可以说是用了各种花里胡哨的方式最终获得了一个带有准确参数的似然模型（或者说是一个概率密度函数），而在这一章，我们不再聚焦于似然问题，而是去看看在解决了似然问题之后的另一问题——**抽样问题**，换种说法，在这一章我们要讨论的是我们似然出来的这个概率模型，到底应该怎么用？

回到我们最经典的那个问题，现在我要用一个高斯模型对全世界的人的身高进行建模，如果你有印象，我[曾经说过](#高斯混合模型（GMM）)，假如你确定了高斯模型的参数，而全人类的身高分布又完全符合一个完美的高斯模型，那么接下来你再也不用头疼于样本数的问题了，拿着你似然出来高斯模型，你想要多少个身高数据，我就可以给你生成多少个数据

听起来好像挺不错，但是当时的你是否有想过，给定一个高斯分布的概率密度函数，我们该如何从其中抽样出所谓的无限多的样本呢？

### 累积分布函数（CDF）

听起来好像是一个很简单的问题，但是仔细一想感觉好像又有点不对，确实，目前我们好像没什么办法严谨的通过一个概率密度函数去抽样样本，你让我凭感觉抽当然可以，我也知道高斯分布靠近均值周围的点取到的概率高，边缘的点取到的概率低，但是你真的让我抽一万个或者更多点，我确实没有办法保证这些点能够真的符合求解出来的这个高斯分布，没错，此时，你就遇到了继似然问题之后的又一个问题——**抽样问题**

明明是看上去一个非常简单的问题，怎么就碰到困难了呢，在此之前，我们不妨看一个非常简单的问题——如何仅用最基础的随机种子（也就是几乎所有编程语言都会提供的`rand()`函数，即它会返回一个$[0,1)$范围内的随机数）取一个$[0,2)$范围内的随机数？

你说这简单，但凡学过一点编程都知道，我可以让`a=rand()*2`，诶这样每次不就产生$[0,2)$范围内的随机数了吗

是的，甚至我可以说，理解这个生成随机数的方法，你其实就已经掌握了抽样问题的解决方法了，如果你不相信，我不妨换一种问法——在我们可以从均匀分布概率密度函数$g(x)=\left\{\begin{matrix}
1  & 0\le x< 1\\
0  &other
\end{matrix}\right.$抽样的情况下，如何借助它从另一个不好抽样的均匀分布概率密度函数$f(x)=\left\{\begin{matrix}
\frac{1}{2}  & 0\le x< 2\\
0  &other
\end{matrix}\right.$中抽样出足够多的样本呢？

> 注意，由于概率密度函数只是表示一个抽象分布的一种方式而已，因此在接下来的过程中，我会使用$f,g$来表示这个分布本身，而用$f(x),g(x)$表示这两个分布的概率密度函数

这其实就是把取随机数的问题换了个问法，答案并没有变，你发现自己好像抓到了一丝解决抽样问题的灵感，不要着急，我们来仔细分析一下，你这种似乎是处于直觉的`rand()*2`代码，背后支持它起作用的原理究竟是什么？

现在我们还是用$f(x)$代表需要被抽样的概率密度函数，而用$g(x)$表示我们有能力抽样的概率密度函数（你已经发现了，解决抽样问题一个不可缺少的要素就是这个已知的分布$g$），我们发现，除了这两个函数之外，在这个过程中还隐藏了另一个函数$G(x)=2x$，如果过一遍抽样的前向过程，这个抽样的过程其实是我们先从$p$这个分布里随便抽样出了一个$x'$，接着我们把这个$x'$输到$G(x)$里，$G$的输出才是$f(x)$这个概率密度函数对应本次抽样出的样本点

我们发现，这个$G$好像构建了一个$g$中样本到$f$的样本的映射，也就是说，$g$抽出来的样本通过$G$可以代表$f$，自然的，$f$抽出来的样本假如过$G$的反函数自然也是可以代表$g$的

要搞清楚$G$是什么，我们就得知道这个映射的操作本身具有的含义，还记得吗，我们[曾经说过](#概率密度函数)不同分布的概率密度函数其实可以看作是不同速率的物体经过同一段路程所形成的速度/时间曲线，因此现在请你想象你在一个百米赛跑的现场，在比赛开始之前，你知道在所有跑道的同一个位置都放着一个传感器，它的作用是当该跑道的运动员经过传感器的时候你就会立刻得到这个消息并记录时间，哨声响起，运动员们同时开始跑步，某一时刻，你收到了一号运动员经过的消息，我们称这个时间为$t'$，你得到$t'$的这个过程其实就是一次抽样

而你遇到的问题是，到了比赛结束你居然仅收到了一号运动员经过的消息，其他传感器居然全部坏掉了，但是你的任务就是准确的记录每一个运动员经过传感器的时间，好在每一个运动员都戴了运动手环，上面记录了他们整场跑步的运动/时间曲线，比如一号运动员的速度曲线是$g(t)$，二号运动员的速度曲线是$f(t)$，而你手上有已知的从一号运动员的抽样数据$t'$，你需要得到二号运动员经过传感器的时间$t^*$，这时候你会怎么做呢？
$$
\int^{t^*}_0f(t)\mathrm{d}t=\int^{t'}_0g(t)\mathrm{d}t
$$
没错，因为所有的传感器都被摆在同一个位置，因此所有运动员通过时所跑过的路程一定是相等的，通过联立路程这个相等量，我们就可以用这个式子求解出$t^*$，从而得到二号运动员的时间，将$t'$看作自变量，这个联立式子的化简结果其实就是$t^*=G(t')$

将其反映到所谓的抽样问题上来，我们说过，运动问题中的路程就可以类比为概率问题中的概率，而$G$形成的过程，**其实是借助所有的分布的概率密度函数在定义域范围内的积分恒为$1$而构造出的一条联立式**，具体来说，我们先通过某一个不知名的函数可以从已知的$g(x)$这个概率密度函数所代表的分布中抽样出一个值$x'$，为了将这个抽样值映射到一个无法抽样的$f(x)$上，我们必须**在概率密度函数这个评判维度上**找到可以同时平等的评价$p$和$f$的共同属性，并且找到可以产生唯一映射的函数，它就是从样本取值边界到$x'$的积分$\int_{-\infty}^{x'}g(x)\mathrm{d}x$或$\int_{x'}^{+\infty}g(x)\mathrm{d}x$，因为对于任意一个概率密度函数$f(x)$来说，这两个积分的结果值一定是可以在$f$上通过同样的规则取到唯一的同等值的，正如同在跑步比赛中我们已知的传感器位于跑道的同一位置，于是我们建立了两个分布的一个一对一的映射关系，也就解决了抽样的样本的映射问题

当$g(x)=1$即它表示$[0,1]$之间的均匀分布的概率密度函数时，对于抽样出的$x'$，当我们尝试把在$f(x)$分布上的$x^*=G(x')$表达式写出来的时候，我们发现好像并不好写，反而是$G$的反函数$x'=G^{-1}(x^*)$好像挺好写的
$$
\int^{x^*}_{-\infty}f(x)\mathrm{d}x=\int^{x'}_{-\infty}1\mathrm{d}x\\
\int^{x^*}_{-\infty}f(x)\mathrm{d}x=x'=G^{-1}(x^*)
$$
如果我们令$G^{-1}=F$，我们就得到了有关分布$f$的**累积分布函数（Cumulative distribution function，CDF）**，这是除了概率密度函数之外的另一种描述分布的方式，相信这个函数大家在概率论上也一定学过，甚至在部分教材中，它出现的时间比概率密度函数还要早，而累积分布函数注定和抽样的过程有着密切的关系，我们可以将**累积分布函数的任一一点$x$对应的函数值$F(x)$定义为从这个分布中抽样出一个样本$x'$小于$x$的概率**，即$F(x)=P(x'\le x)$

当时在学这个函数的时候，你们一定疑惑过这个$F$到底为什么要定义成这么一种奇怪的形式，难道大于等于，或者从$x$积到正无穷不行吗，经过这一节之后，你会发现正是由于抽样问题引出的累积分布函数的反函数，而又由于反函数的不好表示，最终变成了如今的累积分布函数，至于大于等于还是小于等于其实都没有本质的关系，最终都能达到抽样样本在不同分布间转换的目的

> 在此之后，我都会用PDF表示概率密度函数，用CDF表示累积分布函数

### 蒙特卡罗方法

构造CDF来解决抽样问题是非常美好的，只不过相信学过高数的人都知道，目前，世界上所有能够求解积分的函数类型少到几只手就可以数得过来，要不为什么你考的高数卷子上积分题目考来考去都是那么几种，这也说明了，积分这种方式处理PDF存在非常大的局限性，因为谁知道你似然出来的模型概率密度函数长什么样，甚至于我就可以跟你明说高斯分布的积分就是没办法用解析式表示的（虽然你可以根据PDF近似画出来CDF大概长什么样）

但是没办法，为了从一个抽样到另一个抽样，绕不过去的就是求积分的过程，这时候要怎么办呢，既然没办法求解精确的积分，那只能退而求其次求解积分的近似解了，如果说使用正常的解析方法（也就是依赖于牛顿-莱布尼兹公式求解出的积分函数）没有办法求解出某个函数积分函数的解析式，并且恰巧我们并不是那么执着于积分的解析式或者并不是那么在意积分结果的精度，这时候就需要使用数值积分的方式来近似获得某个函数的积分值了

而蒙特卡罗方法可以说是数值积分的各种方法中最暴力的一种方法，他唯一仰赖的就是计算机的高速运算的能力来帮助我们求解积分的值，更直白的说，没有计算机的产生，人们可能永远也不会想到这么暴力的方法

它的原理其实非常简单，你甚至不需要任何一点数学的基础就能很快理解，对于蒙特卡罗方法来说，所谓的积分，实际上不就是算一个函数和坐标轴围成的面积吗，算面积好算啊，我拿一个框框把函数当中需要积分的那一段空间直接罩住，然后往这个框框里面均匀随机的撒固定数量为$n$的豆子，撒完之后数一下落在函数范围内的豆子个数$n'$，再算一算框框的面积$S$，然后我们就可以知道这一段积分的面积$S'\approx S\frac{n'}{n}$，并且你撒的豆子越多，这个结果就越接近于真实的积分值

![img](https://upload.wikimedia.org/wikipedia/commons/thumb/8/84/Pi_30K.gif/220px-Pi_30K.gif)

> 如图，使用蒙特卡罗方法估算π值，在放置30000个随机点后，π的估算值与真实值相差0.07%，效果还是不错的
>
> 并且这个误差实际上受计算机本身取随机数的局限性的影响，如果存在一个真正意义上能够在任意实数范围内随机取值的函数，那么计算结果的精度是随着取值的多少递增的，理想情况下，蒙特卡罗方法的结果实际上就是真正的积分结果

当然，数值积分的方法也是存在十分大的局限性的，最明显的一点就是每算一个积分我们都需要撒一次豆子，并不存在归纳而得的解析式，也就不存在能够一步到位的结果

### 拒绝采样

你可能发现了，虽然蒙特卡罗方法看上去效率不太高的亚子，但是我们想要采样又必须面对CDF，因此借助蒙特卡罗方法的思想进行采样同样也就变成了一个重要的命题，我们不妨来看看

同样的，我们需要一个可以采样的分布$g(x)$和一个目标采样的分布$f(x)$，注意，此时我们的$f$是没有办法求CDF，因此对于$g$中采样出的点并没有办法进行转换，同时，这种采样方式要求$g(x)$和$f(x)$在$x$的取值范围（也就是定义域）相同，具体来说，拒绝采样的步骤分为以下几步：

1. 确定一个系数$c$，令$c\cdot g(x)$的函数值恒大于$f(x)$，这也就是蒙特卡罗方法的那个把所有积分区域框住的框框
2. 接着从$g(x)$里面随机抽样一个$x'$，有了$x'$，我们就有了一个$c\cdot g(x')$，这时候我们还需要在$[0,1]$上取一个随机值$u$
3. 接下来我们比较$u\cdot c \cdot g(x')$和$f(x')$的大小，假如前者小于后者就认为本次抽样的$x'$成功，接受此抽样，否则就丢掉这个$x'$，拒绝该采样，循环1,2两步直到取到一个被接受的$x'$

![image-20230625114914512](https://s2.loli.net/2023/06/25/t4lCR2JMuj7H9Bo.png)

> 如图我们将蓝色直线定义为$g(x)$，绿色直线定义为$f(x)$，若取样点F落在紫色线段上就拒绝，落在橙色线段上就接受，其中浅蓝色区域（包含浅绿色区域）指$g(x)$在定义域内的积分结果，绿色则是$f(x)$的积分区域
>
> 可以发现，我们想要用拒绝采样的方式取$n$个点，那么实际的采样数量一定是大于等于$n$的，因为在通常情况下，会存在某些采样点被拒绝，从而需要重新采样

#### 拒绝采样可行性证明

你肯定会疑问，这种奇怪的方法真的是对的吗，这种采样方法看上去确实有点神奇了，他凭什么是对的呢

要放在以前，我们确实拿这么一个问题没什么办法，但是由于我们已经说明了CDF就是唯一正确的评价取样正确性的方法，因此我们只需要证明通过这种方法采样出来的样本点的分布的CDF和$f(x)$的CDF是一样的就行了

如果用数学式子表达，我们令$f(x),g(x)$的CDF是$F(x),G(x)$，那么我们的目的就是要证明这种方法取出来的样本点$x'$，对于$f(x)$定义域上的任一$x$，一定得满足$P(x'\le x)=F(x)$，如果忘了为什么要这么证，可以回看一下[CDF那一节](#累积分布函数（CDF）)

不过因为我们的$x'$取值是存在判断条件的，即需要满足$u\cdot c \cdot g(x')\le f(x')$，并且其中的$x'$应该保证是从$g$里面取出来的，因此其实我们需要证明的概率问题是一个条件概率问题，总的来说，我们需要证明的条件是$P(x'\le x\mid u\cdot c \cdot p(x')\le f(x'))=F(x)$

1. 为了接下来的推导方便，我令$A=\{x'\le x\},B=\{u\cdot c \cdot g(x')\le f(x')\}$，那么问题题就变成了证明$P(A\mid B)=F(x)$，而在条件概率不好求解的时候，一个自然而然的想法就是贝叶斯公式，因此求解的目标可以进一步化简为$P(A\mid B)=\frac{P(A,B)}{P(B)}=F(x)$

1. 在求解之前，我们得想想怎么把“$x'$是$g$分布的抽样”这个信息加入到我们求解的问题中，首先，我们知道$P(A)=G(x)=\int_xg(x)\mathrm dx$，这没什么问题，本身就是CDF的定义

1. 但是$P(B)$就比较有意思了，我们会发现，我们可以把$P(B)$看成一个边缘概率，但是并不是联合概率$P(A,B)$的边缘概率，而是$P(B,C=\{x'从g分布中抽样\})$的边缘概率，那么我们根据边缘概率的计算公式，可以得到$P(B)=\int_xP(B\mid x'=x)p(x'=x)\mathrm dx$，其中$p(x'=x)=g(x)$这是显然的，毕竟这就是PDF的定义，而后面的$P(B\mid x'=x)=P(u\le \frac{f(x')}{c \cdot g(x')}\mid x'=x)=\frac{f(x)}{c \cdot g(x)}$也是容易理解的，主要原因就是$u$来自于一个$[0,1]$的均匀分布，他的CDF是$U(u)=P(u\le x)=x$，综上所述，我们可以得到$P(B)$

   $$
   \begin{align*}
   P(B)&=\int_x\frac{f(x)}{c \cdot g(x)}g(x)\mathrm{d}x\\
   &=\frac{1}{c}\int_{x}f(x)\mathrm{d}x\\
   &=\frac{1}{c}
   \end{align*}
   $$

   > 顺带一提，这条式子$P(B)=\int_xP(B\mid x)p(x)\mathrm dx$是否让你想到了函数期望，没错，所谓的边缘概率，我们完全可以将其看作是分布$B$对$g$抽样的$x$的条件概率在分布$g$上的期望，听起来确实有点绕，但是也可以给我们另一种理解函数期望的方式

1. 接着就是$P(A,B)$了，其实在理解了如何求解$P(B)$之后，$P(A,B)$只会更加简单，主要还是因为分布$A$本身就是$x'\le x$，我们不需要假设新的分布$C$，根据$P(A,B)=P(B\mid A)P(A)$，我们可以通过对$A$求积分求解结果
   $$
   \begin{align*}
   P(A,B)&=P(\frac{f(x')}{c \cdot g(x')}\mid x'\le x)P(x'\le x)\\
   &=\int_{-\infty}^{x}P(u\le \frac{f(x')}{c \cdot g(x')}\mid x'= w)p(x'=w)\mathrm{d}w\\
   &=\int_{-\infty}^{x}\frac{f(w)}{c \cdot g(w)}g(w)\mathrm{d}w\\
   &=\int_{-\infty}^{x}\frac{f(w)}{c}\mathrm{d}w\\
   &=\frac{F(x)}{c}
   \end{align*}
   $$

5. 我们发现，对分布$A$的积分结果让我们得到了$F(x)$，接下来只需要将结果代入最初的表达式
   $$
   P(A\mid B)=\frac{P(A,B)}{P(B)}=\frac{F(x)}{c}c=F(x)
   $$
...
拒绝采样看上去非常美好，然而你是否发现了，到目前为止，我们讨论的都完全是一维的情况，也就是说假设我对身高进行建模，此时我只考虑如何能抽出一大堆身高样本，但是在大多数场景下，我们的模型是一个高维的分布（比如一张图片中的一大堆像素，代表一个人的一大堆特征等等），在考虑如何从一个高维的模型中进行抽样之前，我们不妨从根本上看看拒绝采样是否存在什么非常严重的问题

你注意到了，在拒绝采样的过程中，我们不可避免的需要事先构造出一个可以采样的$g(x)$并且保证$g(x)$能够覆盖住目标分布的PDF，为了抽样的方便，一个通常的$g(x)$往往就是能够刚好包裹住目的分布的均匀分布，并且你会发现，上一节中我们求解的$P(B)$我们也说过他表示一个期望，而更加具体来说，它其实表示的实际含义正是**每次抽样点被接受概率的期望**，而我们推导出的$P(B)=\frac{1}{c}$，体现出我们的$c$设置的越大，每次抽样就越难以被接受，或者说，由于所有的PDF积分都等于1，乘以$c$之后自然积分也为$c$，$P(B)$告诉我们，**我们覆盖在目标分布的$g(x)$面积越大，拒绝采样的效率就越低，越不容易采样出可以被接受的样本点**

接下来，我们就从面积的角度，来看看高维下的面积，或者说体积会有什么奇怪的变化

![image-20230630225045141](https://s2.loli.net/2023/06/30/FCHnexS1r7RaItg.png)

为了推导的方便，我选择的是最简单的方形和圆形模拟实际情况中的$f(x),g(x)$，非常容易就可以得出，这两个分布的面积比$\frac{1}{c}=\frac{0.5^2\pi}{1*1}$，这个结果大概是$0.785$，看上去还不错，意味着平均我们抽样十次能被接收整整八次

但是现在我们有两维特征，上面的几何图形变成了一个球放在立方体里面的样子（你可以看着上面的平面图想象一下），我们也学过球体的体积公式为$\frac{4}{3}πr^3$，我们不妨求一求在二维的情况下的体积比$\frac{1}{c}=\frac{\frac{4}{3}0.5^3\pi}{1*1*1}\approx0.52$，诶你发现了，当我们给样本多加了一维，同样是和球体完全相切的立方体，他们的体积比居然变大了，意味着如果我们用它来生成二维点，差不多平均采样两次才能成功一次，比起一维将近$80\%$的概率显然低了不少

不过这还不是最可怕的，假设我们把同样的这个几何体拓展到$n$维，已知超球体的体积公式为$V_n=\frac {\pi^ {\frac n 2}} {\Gamma (\frac n 2 + 1)} r^n$，看上去挺复杂的，但是先不管别的，你发现了其中的半径$r$一项是指数级别的，并且我告诉你前面的系数级数一定大于负指数级别，这意味着当$n$趋向于无穷的时候，半径小于$1$的超球体的体积是趋近于$0$的

同时，我们还发现无论$n$升到多少维，超立方体的体积是恒为$1$的，也就是说面对高维样本点，我们这个模型的体积比$\frac{1}{c}$是以接近指数的速度趋于$0$的，或许你只是想生成一个有五个特征的样本点，但是用拒绝采样的方法，或许采样几万个点都接受不了一个，换而言之，在相同模型下，**拒绝采样的时间复杂度和样本维数呈指数上升，我们称这种情况为维数灾难**

更何况实际情况中的模型$f(x)$还不会是那么工整的一个球，我们的$g(x)$更不会这么完美的和它相切，这样一来，即使是一维我们采样的成功率都堪忧，可以说，拒绝采样根本没有处理高维数据的能力

### 马尔可夫链

传统版本的蒙特卡罗方法在高维的抽样问题中碰了壁，意味着即使是蒙特卡罗也需要与时俱进，不过在讨论应该怎么变化之前，我们需要先来了解一下另一个非常有意思的知识，也就是MCMC的另一块拼图——马尔可夫链

#### 转移概率矩阵

现在请你想象一个场景，现在在你面前有一个会发红黄蓝三种颜色光的灯泡，并且每过一秒钟这个灯光会换一个颜色，但是这个灯泡很有意思，下一秒将要切换的颜色和当前展示的颜色有关，比如，当当前显示的颜色是红色的时候，下一秒将会有$60\%$的概率维持红色不变，而变成黄色或者蓝色的概率各为$20\%$，处于其他颜色的时候与此类似都维持一个已知的转移概率，为了表述方便，我可以将其画在一张状态图中

![image-20230702161952642](../../../.config/Typora/typora-user-images/image-20230702161952642.png)

现在问题来了，已知在某一时刻灯泡的颜色是红色，请问在经过了两秒或者说两次变换之后，灯泡颜色为黄色的概率是多少？

这个问题还是比较简单的，灯光变了两次，根据状态图，从红色开始，我们可以知道第一次变化之后灯光为红黄蓝三种颜色的概率分别是$[0.6,0.2,0.2]$，由于我们只需要知道第二秒时灯泡为黄色的概率，因此对于第一次变化后每种颜色的概率乘上其转移到黄色的概率就可以获得结果，即答案为$0.6*0.2+0.2*0.3+0.2*0.5=0.28$

事实上，为了求解的方便，我们可以把状态图里面的各个概率用一个方阵表示，我们称之为**转移概率矩阵**
$$
P=\begin{bmatrix}
0.6  & 0.2 & 0.2\\
 0.5 & 0.3 & 0.2\\
...
\end{bmatrix}
$$
比如，矩阵的第一行就表示当前为红色的时候下一秒变成红，黄，蓝各自的概率，这样，面对同样的问题，我们只需要将初始状态比如红色就是$a_0=[1,0,0]$，然后我们将初始状态向量$a_0$和转移概率矩阵$P$做矩阵乘法，就可以得到第一秒的状态概率向量$a_1=[0.6,0.2,0.2]$可以发现和我们自己推出来的第一秒概率是一致的，接下来我们将$a_1$继续和$P$相乘，得到第二秒的状态概率向量$a_2=[0.54,0.28,0.18]$，取出其中的第二位就是我们的答案$0.28$，而剩余的两项则分别是经过了两秒之后显示其他两种颜色的概率

因此所谓马尔可夫链，其实更像一个“马尔可夫图”，他的结构可以通过一个状态图或者一个转移概率矩阵来表示，在已知初始状态的条件下，我们可以通过矩阵的连乘来表示第任意次转移之后每个状态的概率

#### 平稳分布

马尔可夫链本身确实是一个非常容易理解的结构，但是基于这种简单的结构将会推导出一些非常有意思的结论，比如说现在我用一个罩子把上面这个灯泡罩住，让你猜我打开罩子的一瞬间的灯泡颜色是什么，请问你会猜哪种颜色呢？

听上去好像有点困难，毕竟我既不知道一开始的颜色，也不知道过了几秒钟，那就没办法算出最后的概率，没办法算概率，自然没办法通过相对比较高的概率值去预测颜色，一切都是那么正常

不过既然问题被提出了，自然就是有解决办法的，如果我告诉你，无论是起始状态还是持续时间都不重要，一条马尔可夫链在经过足够多次转移之后，每一个状态的概率会收敛于一个一样的值呢？

听上去挺不可思议的，在此处我们不会严谨的证明，但是我们可以举几个例子，比如你确实没看到一开始灯的颜色，但是被告知在第0秒的时候三种颜色灯的概率是$[0.1,0.4,0.5]$，即有百分之十的概率是红灯，百分之四十的概率是黄灯，百分之五十的概率是蓝灯，现在我们来模拟一下连续闪一百次也就是一百秒之后各个颜色的灯出现的概率是多少

由于手动乘一百次确实有点弱智，所以我们可以写一个简单的程序来模拟一下

```python
transfer_matrix = np.array([[0.6, 0.2, 0.2], [0.5, 0.3, 0.2], [0.4, 0.5, 0.1]])
start_matrix = np.array([[0.1, 0.4, 0.5]])
for i in range(100):
    start_matrix = np.dot(start_matrix, transfer_matrix)  # 这个dot是矩阵叉乘不是点乘
print(start_matrix)
```

最后的结果大概是$[0.54, 0.28, 0.18]$，诶你发现这玩意好像似曾相识啊，这个概率和我们连乘两次的结果怎么一样的，你不要拿现有的数据骗我

不信不要紧，拿着上面的程序，你自己随便修改`start_matrix`，只要这个向量内的值累加为$1$，也就是是一个概率分布，那么无论你怎么乘，再多乘多少次，最后的结果一定都约等于$[0.54, 0.28, 0.18]$，我们称**一条马尔可夫链最终收敛至的这个分布为平稳分布**

> 不过并不是所有马尔可夫链都遵循这个性质，对于马尔可夫链来说也分为周期性和非周期性两种，只有非周期的马尔可夫链才具有平稳分布的性质，至于判断非周期性的依据其实是马尔可夫链中的每一个节点回到自己的所有路径最大公约数为1
>
> ![img](https://picx.zhimg.com/80/v2-b8d58610e38787c9527efa151b10bf71_720w.webp?source=1940ef5c)
>
> 比如这个就是一条周期性的马尔可夫链，因为0可以通过010回到自己，路径长度为2，还可以通过01210回到自己，路径长度为4，并且没有其他不重复路径，而2和4的最大公约数为2不为1，所以马尔可夫链是周期的，对于此类马尔可夫链是输入敏感的，即输入的分布会影响到最终的平稳分布
>
> 可以发现周期性的马尔可夫链比起非周期性的要更加特殊也更少，在接下来的内容中，我们默认使用的均为非周期的马尔可夫链，即下述的马尔可夫链均存在唯一平稳分布

#### 状态转移采样

接下来我们不妨来剖析一下这个灯泡的状态转移过程，假设我们要制作一个这样的灯泡，需要如何设计其逻辑代码呢

1. 首先我们需要指定灯泡的初始颜色，因为没有什么限制，所以一般我们随机从红黄蓝中抽样出一个初始颜色，换句话说，在第一步，我们从一个均匀分布$[0.33,0.33,0.33]$中进行了一次抽样，假设我们本次抽样出了红色
2. 接着等待了一秒之后，灯泡的程序内部一定有一个转移概率矩阵，那么我们就可以找到红色所属的那一行，我们同样可以将其看作一个分布$[0.6,0.2,0.2]$，如何从这个分布中抽样呢，其实我们可以在$[0,1]$内生成一个随机数，接着看看生成的随机数落在$0\sim0.6,0.6\sim0.8,0.8\sim1$的哪个范围内从而决定输出的颜色
3. 假设第一秒转换到了黄色，那么下一秒则需要在$[0.5,0.3,0.2]$这个概率分布中进行采样，重复这个过程，我们就可以实现灯光的转换功能了

可以发现，在这个过程中涉及到了两个采样过程，第一个是初始化的时候使用的从任意分布中进行的采样，而第二个则是根据当前状态从转移概率矩阵定义的$n$个分布中选择一个分布进行采样，其中$n$表示状态总量

那么问题来了，假如从初始化开始，每过一秒我就记录一次灯光的颜色，当经过了足够长的时间之后，请问我把这段时间记录的所有的颜色汇总起来，请问它将满足什么分布呢？

没错假如你反应快你会发现这堆样本点一定是满足平稳分布的，假设你脑子还没转过来，那么一个最简单的理解方式是把整个灯泡都看作是一个黑盒，那么无论什么时候你对这个灯泡进行观测，根据我们在平稳分布的推理，本次观测的观察到三种颜色的概率一定是满足$[0.54, 0.28, 0.18]$这个平稳分布的，在我们进行了足够多次的观测之后，最终样本点的分布满足平稳分布也一定是必然的

如果你还是不相信，你同样可以写一个程序来证明

```python
transfer_matrix = np.array([[0.6, 0.2, 0.2], [0.5, 0.3, 0.2], [0.4, 0.5, 0.1]])
gather, count = [0], 10000
for i in range(count):
    gather.append(np.random.choice(range(3), p=transfer_matrix[gather[i]]))
print(np.bincount(gather) / count)
```

> 其中0,1,2分别表示红黄蓝三种颜色，我们初始状态从红色0开始，`choice`函数可以从指定`p`的离散概率分布中抽样点

不知道你有没有发现，从头到尾我们的程序代码就没有包含一丝一毫和平稳分布有关的东西，我们只是知道了一个不明所以的矩阵，然后随便指定一个初始状态，通过在这个矩阵上所指定的状态上跳来跳去，最终取出的样本居然和平稳分布中取出的样本点是完全一样的

你可能不知道这意味着什么，这意味着假如平稳分布是一个无法积分也就是没法抽样的分布，但是如果存在某种方法，我们可以求出这个没法抽样分布的转移概率矩阵，我们就有办法从这个分布中进行抽样！

#### 连续马尔可夫链

至此我们终于抓到了利用马尔可夫链解决抽样问题的眉目了，没错，所谓MCMC，所解决的最重要的一个问题就是如何求解出某一个无法抽样分布的转移概率矩阵，不过你可能发现了，在此之前我们讨论的其实都是离散的概率分布，对于此类分布其实并不存在无法抽样的问题，毕竟每一种情况（状态）的概率值都写出来了，构造马尔可夫链只能说多此一举，因此为了更好的解决我们在一开始提出的问题，我们需要将马尔可夫链推广到连续的概率分布上去
